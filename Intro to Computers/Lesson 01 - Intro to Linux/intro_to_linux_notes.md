...
4. Change directory into the newly cloned folder             #cd AI2C-IntroToLinux
...
3. Go back to the parent directory.                         #cd ..
...
1. Copy `file2.txt` into the `docs` directory.              #cp file2.txt ./docs
...
4. Delete `newfile.txt`.                                                #rm newfile.txt
   

### Exercise 4: Permissions and Execution

1. View permissions of `scripts/hello.sh`.                             #ls -l scripts/hello.sh
   

2. Make `hello.sh` executable and run it.                          #chmod u+x scripts/hello.sh
                                                                  #./scripts/hello.sh
...
1. Search for the word "Linux" in `file1.txt`.                         #grep "Linux" file2.txt
   

2. Search for lines **not** containing "Linux".                   #grep -v "Linux" file2.txt  
   

3. Search recursively in all `.txt` files under the current directory.
                                                            #grep -r --include="*.txt" "Linux"
   

4. Find lines starting with the word "Note".                 #grep -r --include="*.txt" '^Note'
...
1. Connect to the server that your instructor provided with the correct credentials.
       #ssh user4@46.225.20.198
       #password4

2. Run a remote command (e.g., list home directory files).            #ls
      # if I exit first, ssh user@hostname ls ~

3. Copy a local file to the remote server.
      # exit
      # scp file2.txt user4@46.225.20.198

4. Leave a message for your peers!
      # touch message_for_peers.txt
      # echo 'message for my peers' >> message_for_peers.txt
---
